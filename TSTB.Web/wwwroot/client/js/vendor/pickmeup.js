...
(function (F, y) {
		...
		for (var w = 0; w < b.calendars; w++) {
			...
			(function () {
				...
				for (m = 0; 12 > m; ++m) {
					...
					k.__pickmeup_year = c;
					(b.min && (c < d || (m < e && c === d))) || (b.max && (c > g || (m > h && c >= g)))
						? l(k, 'pmu-disabled')
	...
	function O(a, c) {
		...
		c = H(b);
		B(a, 'input') &&
			(a.value = 'single' === b.mode ? c.formatted_date : c.formatted_date.join(b.separator));
		C(a, 'change', c);
		...
		B(b.parentElement, 'nav')
			? k(b, 'pmu-month')
				? (p(a.current, d - Math.floor(a.calendars / 2)),
				  k(c, 'pmu-view-years')
						? ((a.current =
								'single' !== a.mode ? new Date(a.date[a.date.length - 1]) : new Date(a.date)),
						  a.select_day
								? (c.classList.remove('pmu-view-years'), l(c, 'pmu-view-days'))
								: a.select_month && (c.classList.remove('pmu-view-years'), l(c, 'pmu-view-months')))
						: k(c, 'pmu-view-months')
						? a.select_year
						...
						: k(c, 'pmu-view-days') &&
						  (a.select_month
								...
								: a.select_year && (c.classList.remove('pmu-view-days'), l(c, 'pmu-view-years'))))
				: k(b, 'pmu-prev')
				? a.bound.prev(!1)
				: a.bound.next(!1)
			: k(c, 'pmu-view-years')
	...
	function W(a, c, b) {
			...
			if (((d.date = t(c, d)), 'single' !== d.mode))
				for (
					d.date instanceof Array
						...
						: ((d.date = [d.date]), 'range' === d.mode && d.date.push(t(d.date[0], d))),
						c = 0;
					c < d.date.length;
					++c
				)
		...
		B(a, 'input') &&
			...
			((b = H(d)),
			(c = a.value),
			(d = 'single' === d.mode ? b.formatted_date : b.formatted_date.join(d.separator)),
			c || C(a, 'change', b),
			c !== d && (a.value = d));
	}
	function X(a) {
